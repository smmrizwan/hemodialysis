...
if ($text_format) {
    ...
    echo "=======================================================\n";
    echo "PATIENT'S PERIODIC MANAGEMENT PLAN\n";
    echo "=======================================================\n\n";
    echo "Date: " . format_date($patient['created_at']) . "\n\n";
    ...
    echo "Blood Group: " . safe_value($patient['blood_group']) ."\n";
    ...
    if (isset($patient['bsa']) && $patient['bsa']) {
        ...
        echo "BMI: " . safe_value($patient['bmi']) . " Kg/m²\n";
echo "\n";
    }
   
 	if (isset($patient['dialysis_initiation_date']) && $patient['dialysis_initiation_date']) {
		echo "Dialysis Initiation Date: " . format_date($patient['dialysis_initiation_date']) . "\n";
echo "\n";
	}
 	if (isset($patient['dialysis_years']) && $patient['dialysis_years']) {
		echo "Dialysis Duration in Years: " . safe_value($patient['dialysis_years']) . " years\n";
echo "\n";
	}
if (isset($patient['pre_map']) && $patient['pre_map']) {
		echo "Pre Dialysis MAP: " . safe_value($patient['pre_map']) . "\n";
echo "\n";
	}
if (isset($patient['post_map']) && $patient['post_map']) {
		echo "Post Dialysis MAP: " . safe_value($patient['post_map']) . "\n";
echo "\n";
	}


if (isset($patient['chronic_problems']) && $patient['chronic_problems']) {
		echo "List of Chronic Problems: " . safe_value($patient['chronic_problems']) . "\n";
echo "\n";
        }
if (isset($patient['acute_problems']) && $patient['acute_problems']) {
		echo "List of Acute Problems: " . safe_value($patient['acute_problems']) . "\n";
echo "\n";
        }
...
// History of Peritoneal Dialysis
   echo "MEDICAL BACKGROUND:\n";
        if (isset($medical_background['history_pd']) && $medical_background['history_pd']) {
        echo "History of Peritoneal Dialysis: Yes \n";
        echo "PD Start Date: " . format_date($medical_background['pd_start_date']) . "\n";
        echo "PD End Date: " . format_date($medical_background['pd_end_date']) . "\n";
    } else {
        echo "History of Peritoneal Dialysis: No \n";
echo "\n";
    }
echo "\n";
    // History of Transplant
    if (isset($medical_background['history_transplant']) && $medical_background['history_transplant']) {
        echo "History of Transplant: Yes \n";
        echo "Transplant Start Date: " . format_date($medical_background['transplant_start_date']) . "\n";
        echo "Transplant End Date: " . format_date($medical_background['transplant_end_date']) . "\n";
    } else {
        echo "History of Transplant: No \n";
echo "\n";
    }
echo "\n";
    // Diabetes Mellitus
    if (isset($medical_background['history_dm']) && $medical_background['history_dm']) {
        echo "Diabetes Mellitus: Yes \n";
        echo "DM Duration (years): " . safe_value($medical_background['dm_duration_years']) . "\n";
    } else {
        echo "Diabetes Mellitus: No \n";
echo "\n";
    }
echo "\n";
    // Hypertension
    if (isset($medical_background['history_htn']) && $medical_background['history_htn']) {
        echo "Hypertension: Yes \n";
        echo "HTN Duration (years): " . safe_value($medical_background['htn_duration_years']) . "\n";
    } else {
        echo "Hypertension: No \n";
echo "\n";
    }
echo "\n";
    // Cardiac History
    if (isset($medical_background['history_cardiac']) && $medical_background['history_cardiac']) {
        echo "Cardiac History: Yes \n";
        echo "Cardiac Duration (years): " . safe_value($medical_background['cardiac_duration_years']) . "\n";
    } else {
        echo "Cardiac History: No \n";
echo "\n";
    }
echo "\n";
    // Residual Renal Function
    if (isset($medical_background['residual_renal_function']) && $medical_background['residual_renal_function']) {
        echo "Residual Renal Function Present: Yes \n";
        echo "Residual Urine Output (mL): " . safe_value($medical_background['residual_urine_ml']) . "\n";
    } else {
        echo "Residual Renal Function Present: No \n";
echo "\n";
    }
echo "\n";
    // Primary Cause of CKD
    echo "Primary Cause of CKD:  \n" . safe_value($medical_background['primary_ckd_cause']) . "\n";
echo "\n";

    // Last Ultrasound
    echo "Last Renal Ultrasound Date: " . format_date($medical_background['last_us_date']) . "\n";
    echo "Last US Findings:  \n" . safe_value($medical_background['last_us_findings']) . "\n";
echo "\n";

    // Last Echocardiogram
    echo "Last Echocardiogram Date: " . format_date($medical_background['last_echo_date']) . "\n";
    echo "Last Echo Findings:  \n" . safe_value($medical_background['last_echo_findings']) . "\n";
echo "\n";

    // Other History
    echo "Other Relevant History:  \n" . safe_value($medical_background['other_history']) . "\n";
echo "\n";

} else {
    echo "No medical background data available.\n";
}
 
    
    // Catheter Infections
    if (!empty($catheter_infections)) {
        echo "# HISTORY OF CATHETER REALTED INFECTION:\n";
        echo "--------------------\n";
        foreach (array_slice($catheter_infections, 0, 7) as $infection) {
    ...
    echo "No catheter infections recorded.\n";
}
...
if ($complications) {
    // Hypotension
    echo "Hypotension: " . ($complications['hypotension'] ? "Yes" : "No") . "\n";

    // Hypertension
    echo "Hypertension: " . ($complications['hypertension'] ? "Yes" : "No") . "\n";

    // Muscle Cramps
    echo "Muscle Cramps: " . ($complications['muscle_cramps'] ? "Yes" : "No") . "\n";

    // Nausea/Vomiting
    echo "Nausea/Vomiting: " . ($complications['nausea_vomiting'] ? "Yes" : "No") . "\n";

    // Headache
    echo "Headache: " . ($complications['headache'] ? "Yes" : "No") . "\n";

    // Chest Pain
    echo "Chest Pain: " . ($complications['chest_pain'] ? "Yes" : "No") . "\n";

    // Pruritus
    echo "Pruritus: " . ($complications['pruritus'] ? "Yes" : "No") . "\n";

    // Fever/Chills
    echo "Fever/Chills: " . ($complications['fever_chills'] ? "Yes" : "No") . "\n";

    // Dyspnea
    echo "Dyspnea: " . ($complications['dyspnea'] ? "Yes" : "No") . "\n";

    // Arrhythmias
    echo "Arrhythmias: " . ($complications['arrhythmias'] ? "Yes" : "No") . "\n";

} else {
    echo "No dialysis complications recorded.\n";
}
echo "\n";
 ...
 echo "\n";   
    ...
    if (!empty($quarterly_lab_data)) {
        echo "LABORATORY DATA:\n";
        foreach (array_slice($quarterly_lab_data, 0, 3) as $qlab) {
            echo "Labs in a Quarter/Year: " . safe_value($qlab['quarter_year']) . "\n";
...
if (isset($qlab['hb_diff_1_2']) && $qlab['hb_diff_1_2']) {
		echo "Hb Percentage Difference which indicates magnitude of difference(average):  " . safe_value($qlab['hb_diff_1_2']) . " %\n";
	}
if (isset($qlab['hb_change_1_2']) && $qlab['hb_change_1_2']) {
		echo "Hb Percentage Change which indicates increase or decrease from the initial value:  " . safe_value($qlab['hb_change_1_2']) . " %\n";
	}
            // MCV
            if ($qlab['mcv_1'] || $qlab['mcv_2'] || $qlab['mcv_3']) {
                echo "MCV: ";
...
echo "\n";
            ...
            // Pre-dialysis BUN
            if ($qlab['pre_dialysis_bun_1'] || $qlab['pre_dialysis_bun_2'] || $qlab['pre_dialysis_bun_3']) {
		echo "Dialysis Adequacy Parameters: ";   
		echo "\n";             
